...
**Link previews (WhatsApp/Facebook/Twitter)** come from the Open Graph tags in [index.html](index.html), which point at `public/share.jpg` (1200×630) via absolute URLs — crawlers don't resolve relative paths, so the deployed-site URL is hardcoded there. The image is not generated by the build; its design source is [tools/share-image.html](tools/share-image.html), which documents how to re-render it (headless-Chrome screenshot → JPEG under 300KB). After changing key dates or texts, update both the OG tags and the image. Note WhatsApp caches previews per-URL for days — test changes by appending a throwaway query param to the link.
