...
**Time simulation is a hidden Easter egg, not a dev-only build artifact.** A `?now=<ISO>` query param (parsed in Israel time) overrides the clock in `useNow.ts`, and [DevPhasePanel.tsx](src/components/DevPhasePanel.tsx) renders jump-links to just-before / exactly / just-after each boundary. This works in the production build too — the override only affects the browser that sets it, never shared state. What's gated is the *UI*: `DevPhasePanel` renders `null` until `isRevealed` flips true in [DevPanelProvider.tsx](src/components/DevPanelProvider.tsx) / [useDevPanel.ts](src/utils/useDevPanel.ts), which happens only after 3 clicks within 2s on the "מערכת הבחירות 2026" footer text at the bottom of [NavDrawer.tsx](src/components/NavDrawer.tsx). There is no visible affordance hinting this exists — do not add one.
...
**Link previews (WhatsApp/Facebook/Twitter)** come from the Open Graph tags in [index.html](index.html), via absolute URLs — crawlers don't resolve relative paths, so the deployed-site URL is hardcoded there. `og:image` is deliberately **square** (`public/share-square.jpg`, 1080×1080): WhatsApp renders a center-square crop of the image in its compact preview card, so a wide 1.91:1 banner gets its sides (logo, half the headline) chopped off; a square image survives every WhatsApp layout intact. The wide banner (`public/share.jpg`, 1200×630) is kept for `twitter:image` only. Keep `og:description` ≤ ~80 chars (WhatsApp truncates beyond that) and share images under 300KB (WhatsApp silently drops heavier ones). The images are not generated by the build; design sources are [tools/share-image-square.html](tools/share-image-square.html) and [tools/share-image.html](tools/share-image.html), each documenting how to re-render (headless-Chrome screenshot → JPEG). After changing key dates or texts, update both the OG tags and the images. Note WhatsApp caches previews per-URL for days — test changes by appending a throwaway query param to the link.
